...
2. [Configure the radio](https://meshtastic.org/docs/configuration/radio/), we recommend a private encrypted channel.
    a. Use the [client role](https://meshtastic.org/docs/configuration/radio/device/#role-comparison).
    b. Use the [LOCAL_ONLY rebroadcast mode](https://meshtastic.org/docs/configuration/radio/device/#rebroadcast-mode)
    c. Add a channel named `serial` without password, it'll be used to transport punches through LoRa.
3. Attach SportIdent's SRR module to a UART port. Configure it using instructions below.
...
#### RAK4631
For RAK 4631, we recommend to use UART1: RXD1 and TXD1.

```sh
meshtastic --set serial.rxd 15 --set serial.txd 16
```

You can also use UART0 at RXD1 19 and TXD1 20;

#### Lilygo T-Beam
We recommend use RXD 13 and TXD 14 for Lilygo T-Beam.
